...
module For (C : Preface_specs.CHOICE) : LAWS with module Choice := C = struct
  ...
  let choice_1 () =
    ...
    law ~lhs:("left" =~ lhs) ~rhs:("dimap swap swap % right" =~ rhs)
  ;;
  ...
  let choice_2 () =
    ...
    law
      ~lhs:("map_snd Either.left" =~ lhs)
      ~rhs:("contramap_fst Either.left % left" =~ rhs)
  ;;
  ...
  let choice_3 () =
    ...
    law
      ~lhs:("contramap_fst (Fun.Choice.right f) % left" =~ lhs)
      ~rhs:("map_snd (Fun.Choice.right f) % left" =~ rhs)
  ;;
  ...
  let choice_4 () =
    ...
    law ~lhs:("left % left" =~ lhs) ~rhs:("dimap assoc unassoc % left" =~ rhs)
  ;;
  ...
  let choice_5 () =
    ...
    law ~lhs:("right" =~ lhs) ~rhs:("dimap swap swap % left" =~ rhs)
  ;;
  ...
  let choice_6 () =
    ...
    law
      ~lhs:("map_snd Either.right" =~ lhs)
      ~rhs:("contramap_fst Either.right % right" =~ rhs)
  ;;
  ...
  let choice_7 () =
    ...
    law
      ~lhs:("contramap_fst (Fun.Choice.left f) % right" =~ lhs)
      ~rhs:("map_snd (Fun.Choice.left f) % right" =~ rhs)
  ;;
  ...
  let choice_8 () =
    ...
    law ~lhs:("right % right" =~ lhs) ~rhs:("dimap unassoc assoc % left" =~ rhs)
  ;;
end
