...
(** {1 Tutorial}
    ...
    A [Traversable] is a little different from the modules that we 
    have seen previously ([Functor], [Applicative] and [Monad]).
    It is built using two main components:
    ...
    produces a functor which is to be configured by the subject that we 
    would like to traverse.

    {2 Basics}

    {3 Using an [Applicative] and [List]}

    It is very common to define list as traversable. Here, for example, is 
    a proposal for a traversable list inhabited by applicatives.
    ...
    Let's see how to use the module [List.Traversable]. For example, 
    for [Option]!
    ...
    You can define [Traversable] using [Monad] instead of [Applicative] 
    and like [Functor], [Monad] or [Applicative], a [Traversable] can be 
    define component by component. Offering your own implementation 
...
(** {2 Manual construction} 
    ...
    Advanced way to build a [Traversable], constructing and 
    assembling a component-by-component a traversable. 
