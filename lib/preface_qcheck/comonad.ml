...
module Suite
    (R : Model.COVARIANT_1)
    (F : Preface_specs.COMONAD with type 'a t = 'a R.t)
    (A : Model.T0)
    (B : Model.T0)
    (C : Model.T0)
    (D : Model.T0) =
struct
  ...
  module Laws = Preface_laws.Comonad.For (F)
  ...
  let comonad_2 count =
    let generator =
      Gen.tup2
        (fun1 (R.observable A.observable) B.generator)
  ...
  let comonad_3 count =
    let generator =
      Gen.tup3
        (fun1 (R.observable A.observable) B.generator)
        (fun1 (R.observable C.observable) A.generator)
  ...
  let comonad_6 count =
    let generator =
      Gen.tup4
        (fun1 (R.observable A.observable) B.generator)
        (fun1 (R.observable B.observable) C.generator)
        (fun1 (R.observable C.observable) D.generator)
        (R.generator A.generator)
    in
  ...
  let comonad_9 count =
    ...
    Util.test ~count ~print generator Laws.comonad_9 (fun lhs rhs x ->
        let left = lhs x
        and right = rhs x in
        R.equal (R.equal (R.equal A.equal)) left right )
  ;;
  ...
  let comonad_11 count =
    ...
    Util.test ~count ~print generator Laws.comonad_11 (fun lhs rhs x ->
        let left = lhs x
        and right = rhs x in
        R.equal (R.equal A.equal) left right )
  ;;
