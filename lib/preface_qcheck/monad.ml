...
module Suite
    ...
    (D : Model.T0) =
struct
  module Bind = Bind.Suite (R) (F) (A) (B) (C) (D)
  module Laws = Preface_laws.Monad.For (F)
  ...
  let monad_1 count =
    ...
    Util.test ~count ~print generator Laws.monad_1 (fun lhs rhs x ->
        ...
        && R.equal A.equal left id_left
        && R.equal A.equal right id_right )
  ;;
  ...
  let monad_3 count =
    let generator =
      Gen.tup2 A.generator (fun1 A.observable (R.generator B.generator))
    in
