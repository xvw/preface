...
module Suite
    ...
    (C : Model.T0) =
struct
  module Laws = Preface_laws.Selective.For (F)
  module Applicative = Applicative.Suite (R) (F) (A) (B) (C)

  let print pp = Format.asprintf "%a" (R.pp pp)

  let selective_1 count =
    let generator = R.generator (Util.gen_either A.generator A.generator) in
  ...
  let selective_3 count =
    let generator =
      ...
      and g = R.generator (fun2 C.observable A.observable B.generator) in
      Gen.tup3 (R.generator (Util.gen_either A.generator B.generator)) f g
    in
  ...
  let selective_4 count =
    let generator =
      Gen.tup3
        ...
        (R.generator (Util.gen_either C.generator A.generator))
  ...
  let selective_5 count =
    let generator =
      Gen.tup3
        (fun1 A.observable B.generator)
        (R.generator (Util.gen_either A.generator C.generator))
        (R.generator (fun1 B.observable C.generator))
    in
  ...
  let selective_6 count =
    let generator =
      Gen.tup3
        ...
        (R.generator (Util.gen_either B.generator C.generator))
  ...
  let selective_7 count =
    let generator =
      Gen.tup2
        (R.generator (Util.gen_either A.generator B.generator))
        (fun1 A.observable B.generator)
    in
...
module Suite_rigid
    ...
    (C : Model.T0) =
struct
  module Non_rigid = Suite (R) (F) (A) (B) (C)
  module Laws = Preface_laws.Selective.For_rigid (F)
