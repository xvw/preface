(** Exposes [Try.t]
    ...
    [Validation] errors are not accumulate. The pipelines returns 
    [Ok result] or [Error first_error_occured]. [Try] goes well with 
    monadic composition.

    {2 Sequential pipelining}

    [Try] is usually useful when a collection of functions can fail and we 
    would like to sequence them. For example :
    ...
    We want, if the age is valid, to verify that the name is valid, and if 
    the name is also valid, to produce the following couple: [age, name].

    {[
      let result in_age in_name = 
        let open Preface.Try.Monad.Infix in 
        validate_age in_age 
        >>= fun age  -> validate_name in_name
        >|= fun name -> (age, name) 
    ]}
