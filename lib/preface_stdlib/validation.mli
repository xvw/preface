(** Exposes [Validation.t]
   ...
   {1 Capabilities}
    ...
    Formlet validation is a problem recognized as being complex. Fortunately, 
    20 years ago, a method drawing on application functors was highlighted 
    in the paper
    ...
    - [checked_rules], the "do you accept the rules of the website", which 
    must be true.
      ...
      module Subscription_formlet = struct
        ...
        let validate_age age = 
          if age >= 0 then 
            ...
            Error [ Invalid_age age ]

        let validate_name subject name = 
          if String.length name > 1 then 
            Ok name 
          else
            Error [ Invalid_name (subject, name) ]

        let validate_rules checked = 
          if checked then 
            Ok checked 
          else 
            Error [ Unchecked_rules ]
      end
    ]}

    We can now use our [make] functions and our validation functions to 
    validate the creation of a subscription:
    ...
    When the checkbox "Accept all the rules" is not checked and the 
    [age] is negative!
...
(** [capture f] perform [f] and wrap the result into a [t] if the 
    execution of [f] raise no exception, the result will be [Ok result]
    else, the catched exception if wrapped into [Error exn].
*)
