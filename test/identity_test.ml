...
let identity a =
    Alcotest.testable (pp (Alcotest.pp a)) (eq (=))

let should_replace() =
    let expected = pure 42
    and computed = replace 42 (pure 41)
...
let test_cases =
    let open Alcotest in
        ...
        [
            test_case "Replace"
                `Quick should_replace;
            test_case "Infix replace"
                `Quick should_replace_with_infix_operator;
            test_case "Infix flipped replace"
                `Quick should_flipped_replace_with_infix_operator;
            test_case "Void"
                `Quick should_void;
            test_case "Map"
                `Quick should_map;
            test_case "Infix map"
                `Quick should_map_with_infix_operator;
            test_case "Infix flipped map"
                `Quick should_flipped_map_with_infix_operator;
        ]