...
let should_transform_a_pcdata () =
  ...
  Alcotest.(check (list sax))
    "transform_a_pcdata" expected computed
;;
...
let should_transform_a_tag () =
  ...
  Alcotest.(check (list sax))
    "transform_a_tag" expected computed
;;

let should_transform_a_sequence () =
  let open Writer in
  let expected = Stax.[ Open "A"; Close "A"; Text "Hello World" ]
  and _, computed = run (sax_like Xml.(Seq (Tag ("A", Empty), PCData "Hello World"))) in
  Alcotest.(check (list sax))
...
let cases =
  ...
  [ ("Xml to Stax reader", [
      test_case "Should transform a pcdata" `Quick
                should_transform_a_pcdata
    ; test_case "Should transform a tag" `Quick
