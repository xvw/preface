(** Unit test over the Arrow tutorial from
...
module Circuit = struct
  ...
  module Arrow =
    Preface_make.Arrow.Over_category_and_via_arrow_and_fst
      (Category)
      (struct
        type nonrec ('a, 'b) t = ('a, 'b) t
...
let avg_proc_test () =
  let circuit =
    Circuit.Arrow.(totalf &&& Fun.const 1. ^>> totalf >>> arrow (uncurry ( /. )))
  in
