...
open Applicative.Traversable (Preface_stdlib.Option.Applicative)
...
let test_cases =
  ...
  [
    ( "List Traversable for Option as an Applicative"
    , [
        test_case "Sequence with valid input" `Quick
          should_traverse_without_failure
      ; test_case "Sequence with invalid input" `Quick
