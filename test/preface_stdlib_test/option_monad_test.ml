...
open Preface_stdlib.Option.Monad

let should_bind () =
  let expected = return 42
  and computed = bind (fun x -> return (x + 2)) @@ return 40 in
  Alcotest.(check (option int)) "should_bind" expected computed
...
let should_join () =
  ...
  Alcotest.(check (option int)) "should_join" expected computed
...
let should_void () =
  ...
  Alcotest.(check (option unit)) "should_void" expected computed
...
let should_discard_first_value () =
  ...
  Alcotest.(check (option int)) "should_discard_first_value" expected computed

let should_discard_second_value () =
  let expected = return 42 and computed = return 42 << return "42" in
  Alcotest.(check (option int)) "should_discard_second_value" expected computed
